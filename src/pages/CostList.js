...
const CostList = () => {
    ...
    const { loading, successDelete, costData } = useSelector((state) => state.cost)
            ...
            cell: (row) => {
                return (
                    <div className='d-flex' style={{ alignItems: "center" }}>
                        <Trash onClick={() => deleteCost(`${row._id}`)} style={{ color: 'red' }} className='me-2' size={15} />
                        <a href={`costs/${row._id}`}><Edit size={15} /></a>
                    </div>
                )
            }
        }
